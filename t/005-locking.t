...
main(_) ->
	...
	etap:is(gb_trees:size((hd(emysql_pool_mgr:pools()))#pool.locked), 0, "connections unlocked"),
	
	etap:is(emysql_conn_mgr:waiting(), queue:new(), "waiting queue is empty"),
	
	ConnIDs = [Conn#connection.id || Conn <- queue:to_list((hd(emysql_pool_mgr:pools()))#pool.available)],
	...
	etap:is((hd(emysql_pool_mgr:pools()))#pool.available, queue:new(), "all connections locked"),
	...
	[etap:is(lists:member(Conn#connection.id, ConnIDs), false, "connection has been replaced") || Conn <- queue:to_list((hd(emysql_pool_mgr:pools()))#pool.available)],
